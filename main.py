...
# =========================
# PREDICTION ENGINE (ANTI-INVERSE LOGIC)
# =========================
class PredictionEngine:
    ...
    def get_pattern_signal(self, current_streak_loss):
        ...
        # =========================================================
        # 🔥 STEP 1: CALCULATE "NORMAL" PREDICTION
        # =========================================================
        # সাধারণত আমরা কী ধরতাম?
        normal_prediction = last # Default Trend

        # ZigZag Check (B S B ...)
        if last != prev1 and prev1 != prev2:
            normal_prediction = "SMALL" if last == "BIG" else "BIG"
        # Dragon Check (B B ...)
        elif last == prev1:
            normal_prediction = last

        # =========================================================
        # 🛡️ STEP 2: ANTI-INVERSE LOGIC (THE FIX)
        # =========================================================
        # যদি দেখি টানা ২ বার বা তার বেশি লস হয়েছে, 
        # তার মানে মার্কেট আমাদের লজিকের উল্টা (Inverse) চলছে।
        # তখন আমরাও উল্টা ধরব।
        
        if current_streak_loss >= 2 and current_streak_loss < 5:
            # Flip the normal prediction
            return "SMALL" if normal_prediction == "BIG" else "BIG"

        # =========================================================
        # 🚨 STEP 3: EMERGENCY SAFETY (LOSS 5+)
        # =========================================================
        # যদি ৫ বার লস হয়, তখন আর পন্ডিতি না করে ট্রেন্ড (Last Result) ফলো করা সেফ।
        if current_streak_loss >= 5:
            return last

        # যদি লস কম থাকে (0-1), তবে নরমাল প্রেডিকশনই যাবে।
...
async def start_session(bot):
    ...
    for s in STICKERS["SESSION_START_SEQ"]: await broadcast_sticker(bot, s)
    ...
    while state.running and state.session_id == my_session:
        ...
        if (not state.active) and is_safe_time and (not resolved_this_tick):
            if state.last_signal_issue != current_period:
                ...
                rand_color = random.choice(["GREEN", "RED"]) if state.color_mode else None
                pred_stk, color_stk = choose_pred_stickers(pred)
                await broadcast_sticker(bot, pred_stk)
                if state.color_mode and rand_color:
                    await broadcast_sticker(bot, STICKERS["COLOR_GREEN_OLD"] if rand_color == "GREEN" else STICKERS["COLOR_RED_OLD"])
                await broadcast_message(bot, format_signal(current_period, pred, conf, rand_color))
                checking_ids = {}
                for cid in state.selected_targets:
                    try:
                ...
                state.active = ActiveBet(predicted_issue=current_period, pick=pred, random_color=rand_color, checking_msg_ids=checking_ids)
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "TOGGLE_COLOR":
        ...
        return
    if data == "START:1M":
        if state.running: await stop_session(context.bot, reason="restart_manual")
    ...
    if data == "STOP:GRACEFUL":
        if state.running:
            ...
            if state.streak_loss == 0 and state.active is None: await stop_session(context.bot, reason="graceful_now")
        await q.edit_message_text(panel_text(), parse_mode=ParseMode.HTML, reply_markup=selector_markup())
        return
